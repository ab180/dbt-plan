...
def predict_ddl(
    ...
    Args:
        ...
        base_columns: Columns from base (None if new model or parse failed).
        current_columns: Columns from current (None if removed or parse failed).
    ...
    # New model (no base) → safe, no existing table to alter
    ...
    # Column diff
    base_set = set(base_columns) if base_columns else set()
    current_set = set(current_columns) if current_columns else set()
